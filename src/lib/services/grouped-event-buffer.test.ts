...
describe('buffer instances', () => {
  ...
  it('isolates overlapping event ids and listeners', () => {
    ...
    first.ingestHistoryEvent(makeActivityScheduled(1));

    expect(first.getGroupArray()).toHaveLength(1);
    expect(second.getGroupArray()).toHaveLength(0);
  ...
  it('replaces the compatibility buffer and version synchronously', () => {
    ...
    next.reset(10);
    next.ingestHistoryEvent(makeActivityScheduled(1));
