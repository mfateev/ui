...
export function createGroupedEventBuffer() {
  ...
  function attachFollowerToPool(
    poolIdx: number,
    followerSlotIdx: number,
  ): void {
  ...
  function absorbLiveGroupIntoPool(
    poolIdx: number,
    liveGroup: EventGroup,
  ): void {
    const meta = groupPool[poolIdx];
    ...
    for (const event of liveGroup.eventList) {
      ...
      if (group.eventList.some((existing) => existing.id === event.id))
        continue;
   ...
   */
  function setEstimatedGroupCount(n: number): void {
  ...
  function getLatestEvent(): HistoryEvent | null {
   ...
   */
  function getLatestGroup(): Promise<EventGroup | null> {
   ...
   */
  function onLatestGroup(cb: LatestGroupListener): () => void {
  ...
  function enrichGroups(
    ...
    const byActivityId = new Map(
      pendingActivities.map((p) => [p.activityId, p]),
    );
    ...
    const scanGroup = (group: EventGroup): void => {
      for (const event of group.eventList) {
        ...
        if (
          ...
          if (
            !lastFailedEvent ||
            Number(event.id) > Number(lastFailedEvent.id)
          ) {
   ...
   */
  function getGroupArray(opts?: GetRowsOptions): EventGroup[] {
   ...
   */
  function assignTrackIndices(): void {
  ...
  function getVisibleGroupCount(): number {
  ...
  function _debugEventSlots(): readonly (HistoryEvent | null)[] {
    return eventSlots;
  }
  ...
  function appendLiveEvent(raw: HistoryEvent): boolean {
    ...
    if (!isHead) {
      ...
      if (existingIdx !== -1) {
        liveGroups[existingIdx] = withAddedEvent(
          liveGroups[existingIdx],
          event,
        );
        _liveVersion++;
        invalidateGroupArrayCaches();
  ...
  function getLiveGroupCount(): number {
   ...
   * Returns an unsubscribe function.
   */
  function onLiveGroup(cb: LatestGroupListener): () => void {
...
export const _debugEventSlots = () =>
  activeBufferHandle.current._debugEventSlots();
export const _debugState = () => activeBufferHandle.current._debugState();
export const appendLiveEvent = (
  ...args: Parameters<GroupedEventBuffer['appendLiveEvent']>
) => activeBufferHandle.current.appendLiveEvent(...args);
export const assignTrackIndices = () =>
  activeBufferHandle.current.assignTrackIndices();
export const enrichGroups = (
  ...args: Parameters<GroupedEventBuffer['enrichGroups']>
) => activeBufferHandle.current.enrichGroups(...args);
export const getAscGroupCount = () =>
  activeBufferHandle.current.getAscGroupCount();
export const getDescGroupCount = () =>
  activeBufferHandle.current.getDescGroupCount();
export const getEventArray = () => activeBufferHandle.current.getEventArray();
export const getGroupArray = (
  ...args: Parameters<GroupedEventBuffer['getGroupArray']>
) => activeBufferHandle.current.getGroupArray(...args);
export const getGroupCount = () => activeBufferHandle.current.getGroupCount();
export const getGroupMeta = (
  ...args: Parameters<GroupedEventBuffer['getGroupMeta']>
) => activeBufferHandle.current.getGroupMeta(...args);
export const getLatestEvent = () => activeBufferHandle.current.getLatestEvent();
export const getLatestGroup = () => activeBufferHandle.current.getLatestGroup();
export const getLiveGroupCount = () =>
  activeBufferHandle.current.getLiveGroupCount();
export const getRows = (...args: Parameters<GroupedEventBuffer['getRows']>) =>
  activeBufferHandle.current.getRows(...args);
export const getVisibleGroupCount = () =>
  activeBufferHandle.current.getVisibleGroupCount();
export const getWorkflowTaskFailedEvent = () =>
  activeBufferHandle.current.getWorkflowTaskFailedEvent();
export const isWorkflowTaskGroup = (
  ...args: Parameters<GroupedEventBuffer['isWorkflowTaskGroup']>
) => activeBufferHandle.current.isWorkflowTaskGroup(...args);
export const mergeHeads = () => activeBufferHandle.current.mergeHeads();
export const onLatestGroup = (
  ...args: Parameters<GroupedEventBuffer['onLatestGroup']>
) => activeBufferHandle.current.onLatestGroup(...args);
export const onLiveGroup = (
  ...args: Parameters<GroupedEventBuffer['onLiveGroup']>
) => activeBufferHandle.current.onLiveGroup(...args);
export const processEvent = (
  ...args: Parameters<GroupedEventBuffer['processEvent']>
) => activeBufferHandle.current.processEvent(...args);
export const reset = (...args: Parameters<GroupedEventBuffer['reset']>) =>
  activeBufferHandle.current.reset(...args);
export const resetLive = () => activeBufferHandle.current.resetLive();
export const setEstimatedGroupCount = (
  ...args: Parameters<GroupedEventBuffer['setEstimatedGroupCount']>
) => activeBufferHandle.current.setEstimatedGroupCount(...args);
export const setFailedEvent = (
  ...args: Parameters<GroupedEventBuffer['setFailedEvent']>
) => activeBufferHandle.current.setFailedEvent(...args);
